...
export async function generateBootstrapConfig(deviceInfo: BootstrapRequest) {
  ...
  // 使用新的凭证服务生成动态凭证
  const credentials = await credentialsService.generateDeviceCredentials(
    tenantId, 
    deviceId, 
    deviceType,
    24 // 24小时有效期
  ...
  // 从配置中心获取租户和设备配置
  const [tenantConfig, deviceConfig, mqttConfig, otaConfig] = await Promise.all([
    configManager.getTenantConfig(tenantId),
    configManager.getDeviceConfig(tenantId, deviceType),
    configManager.getMQTTConfig(tenantId, deviceType),
    configManager.getOTAConfig(tenantId, deviceType)
  ]);
  ...
  // 获取OTA决策
  const otaDecision = await getOTADecision(deviceInfo, tenantId, deviceType, otaConfig);
  ...
  const config = {
    cfg: {
      ...
      device: {
        ...
        fw: {
          ...
          min_required: deviceConfig?.maintenance?.minRequiredVersion || "1.0.0"
        },
    ...
    mqtt: {
      ...
      password_expires_at: credentials.expiresAt,
      keepalive: mqttConfig.client.keepAlive,
      clean_start: mqttConfig.client.cleanStart,
      session_expiry: mqttConfig.client.sessionExpiry,
      tls: {
        enabled: mqttConfig.tls.enabled,
        ca_cert_fingerprint: mqttConfig.tls.caCert
      },
      lwt: {
      ...
      qos_retain_policy: mqttConfig.qosRetainPolicy,
      acl: credentials.acl,
      backoff: mqttConfig.backoff
    },
    ...
    policies: {
      ingest_limits: {
        telemetry_qps: deviceConfig.sampling?.telemetryInterval ? 1000 / deviceConfig.sampling.telemetryInterval : 3,
        status_qps: 1,
        event_qps: 5
      },
      retention: tenantConfig.retention,
    ...
    websocket: {
      enabled: tenantConfig.websocket?.enabled || false,
 ...
 * 获取OTA决策 - 使用配置中心
 */
async function getOTADecision(deviceInfo: BootstrapRequest, tenantId: string, deviceType: string, otaConfig: any) {
  const currentVersion = deviceInfo.firmware_version || "1.0.0";
  const latestVersion = "1.2.0";
  const needsUpdate = compareVersions(currentVersion, latestVersion) < 0;
  
  if (!needsUpdate || !otaConfig.enabled) {
    return {
      available: false,
      firmware: null,
      retry: {
        base_ms: otaConfig.download?.retryAttempts || 5000,
        max_ms: otaConfig.download?.timeout || 300000
      }
    };
  }
  
  // 检查OTA约束条件
  const constraints = await getOTAConstraints(tenantId, deviceType, deviceInfo, otaConfig);
  ...
  return {
    ...
    firmware: {
      ...
      url: `${otaConfig.repository.baseUrl}/firmware/${deviceType}/${latestVersion}`,
      checksum: "sha256:abc123def456...",
      size: 2048576,
      release_notes: "固件更新：\n- 优化性能\n- 增强稳定性\n- 修复已知问题",
      force: otaConfig.versioning?.forceUpdate ? 1 : 0,
 ...
 * 获取OTA约束条件 - 使用配置中心
 */
async function getOTAConstraints(tenantId: string, deviceType: string, deviceInfo: BootstrapRequest, otaConfig: any) {
  return {
    min_battery_pct: otaConfig.rollout?.conditions?.minBatteryLevel || 20,
    network: otaConfig.rollout?.conditions?.networkType || ["wifi"],
    time_window: otaConfig.rollout?.conditions?.timeWindow || "02:00-06:00",
    device_type: deviceType,
    tenant_id: tenantId,
    rollout_percentage: otaConfig.rollout?.percentage || 50,
    region: "asia-pacific",
    tags: otaConfig.rollout?.conditions?.deviceTags || ["stable", "production"]
  };
}
